...
class ValueAxisMrtg : RrdGraphConstants {
	...
	internal bool draw() {
		...
		if (im.scaledstep / im.magfact * Math.Max(Math.Abs(im.quadrant), Math.Abs(4 - im.quadrant)) <= 1.0) {
			...
			labfmt = String.Format("%%4.%df", 1 - ((im.scaledstep / im.magfact > 10.0 || Math.Ceiling(im.scaledstep / im.magfact) == im.scaledstep / im.magfact) ? 1 : 0)); //TODO: vdaron - fix fomat string
		}
		...
		for (int i = 0; i <= 4; i++) {
			...
			if (y >= im.yorigin - im.ysize && y <= im.yorigin) {
				...
				worker.drawString(graph_label, xLeft - length - PADDING_VLABEL, y + labelOffset, font, new SolidBrush(fontColor));
				worker.drawLine(xLeft - 2, y, xLeft + 2, y, new Pen(mGridColor, TICK_STROKE));
				worker.drawLine(xRight - 2, y, xRight + 2, y, new Pen(mGridColor, TICK_STROKE)); 
				worker.drawLine(xLeft, y, xRight, y, new Pen(mGridColor, GRID_STROKE)); 
			}
		}
		return true;
	}
