...
class ValueAxis {
	...
	public ValueAxis(ImageParameters im, ImageWorker worker, RrdGraphDef gdef, Mapper mapper) {
		this.im = im;
		this.gdef = gdef;
		this.worker = worker;
		this.mapper = mapper;
	...
	internal bool draw() {
		...
		Color gridColor = gdef.colors[RrdGraphConstants.COLOR_GRID];
		Color mGridColor = gdef.colors[RrdGraphConstants.COLOR_MGRID];
		Color fontColor = gdef.colors[RrdGraphConstants.COLOR_FONT];
		...
		if (Double.IsNaN(im.ygridstep)) {
			if (gdef.altYGrid) {
				...
				if (fractionals < 0) /* small amplitude. */ {
					labfmt = String.Format("%%%d.%df", decimals - fractionals + 1, -fractionals + 1); //TODO: vdaron - fix format string
				}
				else {
					labfmt = String.Format("%%%d.1f", decimals + 1); //TODO: vdaron - fix format string
				}
		...
		for (int i = sgrid; i <= egrid; i++) {
			...
			if (y >= im.yorigin - im.ysize && y <= im.yorigin) {
				if (i % labfact == 0) {
					...
					if (i == 0 || im.symbol == ' ') {
						if (scaledstep < 1) {
							if (i != 0 && gdef.altYGrid) {
								...
								graph_label = String.Format("{0}", scaledstep * i); //TODO: vdaron - fix format string
							}
						}
						else {
							graph_label = String.Format("{0}", scaledstep * i); //TODO: vdaron - fix format string
						...
						if (scaledstep < 1) {
							graph_label = String.Format("{0} {1}", scaledstep * i, im.symbol); //TODO: vdaron - fix format string
						}
						else {
							graph_label = String.Format("{0} {1}", scaledstep * i, im.symbol); //TODO: vdaron - fix format string
						}
					...
					worker.drawString(graph_label, x0 - length - RrdGraphConstants.PADDING_VLABEL, y + labelOffset, font,new SolidBrush(fontColor));
					worker.drawLine(x0 - 2, y, x0 + 2, y,new Pen(mGridColor, RrdGraphConstants.TICK_STROKE));
					worker.drawLine(x1 - 2, y, x1 + 2, y,new Pen( mGridColor, RrdGraphConstants.TICK_STROKE));
					worker.drawLine(x0, y, x1, y, new Pen(mGridColor, RrdGraphConstants.GRID_STROKE));
				}
				else if (!(gdef.noMinorGrid)) {
					worker.drawLine(x0 - 1, y, x0 + 1, y,new Pen( gridColor, RrdGraphConstants.TICK_STROKE));
					worker.drawLine(x1 - 1, y, x1 + 1, y,new Pen( gridColor, RrdGraphConstants.TICK_STROKE));
					worker.drawLine(x0, y, x1, y, new Pen(gridColor, RrdGraphConstants.GRID_STROKE));
				}
			}
		}
		return true;
...
private int findLabelFactor(YLab thisYLab) {
	...
	int fontHeight = (int) Math.Ceiling(worker.getFontHeight(gdef.smallFont));
	for (int j = 0; j < 4; j++) {
