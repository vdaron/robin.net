	...
	internal bool draw() {
		...
		for (double value = Math.Pow(10, Math.Log10(im.minval)
			...
			while (yloglab[minoridx,++i] > 0) {
				...
				worker.drawLine(x0 - 1, y, x0 + 1, y, new Pen(gridColor, TICK_STROKE)); 
				worker.drawLine(x1 - 1, y, x1 + 1, y, new Pen(gridColor, TICK_STROKE)); 
				worker.drawLine(x0, y, x1, y, new Pen(gridColor, GRID_STROKE)); 
			}
		}
		for (double value = Math.Pow(10, Math.Log10(im.minval)
			...
			while (yloglab[majoridx,++i] > 0) {
				...
				worker.drawLine(x0 - 2, y, x0 + 2, y, new Pen(mGridColor, TICK_STROKE));
				worker.drawLine(x1 - 2, y, x1 + 2, y, new Pen(mGridColor, TICK_STROKE));
				worker.drawLine(x0, y, x1, y, new Pen(mGridColor, GRID_STROKE));
				String graph_label = String.Format("%3.0e", value * yloglab[majoridx,i]);//TODO: vdaron - fix format string
				int length = (int) (worker.getStringWidth(graph_label, font));
				worker.drawString(graph_label, x0 - length - PADDING_VLABEL, y + labelOffset, font,new SolidBrush(fontColor));
			}
		}
		return true;
	}
